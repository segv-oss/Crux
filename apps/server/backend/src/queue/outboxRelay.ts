  ...
  public async processBatch(): Promise<number> {
    ...
    try {
      ...
      for (const row of res.rows) {
        try {
          const payload = typeof row.inline_payload === 'string'
          ...
          logger.error({ err, outboxId: row.id, retryCount: row.retry_count }, 'Failed to dispatch outbox event');
...
export function startOutboxRelay(intervalMs: number = 300): () => void {
