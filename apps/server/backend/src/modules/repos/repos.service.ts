...
import { RepositoryDTO, PaginatedResult } from '../../types/index.js';
...
export async function listOrgRepositories(
  ...
  if (cursor) {
    try {
      ...
      if (direction === 'forward') {
        conditions.push(`(r.created_at, r.id) < ($${paramIdx}::timestamptz, $${paramIdx + 1}::text)`);
      } else {
        conditions.push(`(r.created_at, r.id) > ($${paramIdx}::timestamptz, $${paramIdx + 1}::text)`);
      }
  ...
  const resultItems: RepositoryDTO[] = items.map((r) => ({
    id: r.id,
    orgId: r.org_id,
    githubRepoId: parseInt(r.github_repo_id, 10),
    name: r.name,
...
export async function deleteRepository(repoId: string, hard: boolean = false): Promise<void> {
  if (hard) {
    const res = await pool.query(`DELETE FROM repositories WHERE id = $1 RETURNING id`, [repoId]);
    if (res.rowCount === 0) {
      throw new AppError({ status: 404, code: 'REPOSITORY_NOT_FOUND', message: `Repository '${repoId}' not found.` });
    }
  } else {
    const res = await pool.query(
    ...
    if (res.rowCount === 0) {
      throw new AppError({ status: 404, code: 'REPOSITORY_NOT_FOUND', message: `Repository '${repoId}' not found or already deleted.` });
    }
  }
}

export async function restoreRepository(repoId: string): Promise<{ success: boolean; restoredAt: string }> {
