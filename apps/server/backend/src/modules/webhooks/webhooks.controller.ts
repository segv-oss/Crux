...
export async function handleGitHubWebhook(c: Context<AppEnv>) {
  ...
  const secretUsed = webhooksService.verifyHmacSignature(Buffer.from(rawBody), signature, timestamp);
  let payload: any = {};
  try {
  ...
  const result = await webhooksService.processWebhookIngress('github', deliveryId, payload, secretUsed);
...
export async function handleLinearWebhook(c: Context<AppEnv>) {
  ...
  const result = await webhooksService.processWebhookIngress('linear', deliveryId, payload, secretUsed);
  const statusCode = result.status === 'buffered_in_stream' ? 202 : 200;
  return c.json(result, statusCode as any);
}
...
export async function handleSlackWebhook(c: Context<AppEnv>) {
  ...
  const result = await webhooksService.processWebhookIngress('slack', deliveryId, payload, secretUsed);
  const statusCode = result.status === 'buffered_in_stream' ? 202 : 200;
  return c.json(result, statusCode as any);
}
