...
import { OrganizationDTO, PaginatedResult } from '../../types/index.js';
  ...
  if (cursor) {
    try {
      ...
      if (direction === 'forward') {
        conditions.push(`(o.created_at, o.id) < ($${paramIdx}::timestamptz, $${paramIdx + 1}::text)`);
      } else {
        conditions.push(`(o.created_at, o.id) > ($${paramIdx}::timestamptz, $${paramIdx + 1}::text)`);
      }
...
export async function inviteMember(
  ...
  return withTransaction(async (client) => {
    ...
    let userRes = await client.query(`SELECT id, email, name, avatar_url FROM users WHERE email = $1`, [body.email]);
    ...
    return {
      ...
      email: body.email,
      role: body.role,
      createdAt: persistedRow.created_at ? new Date(persistedRow.created_at).toISOString() : new Date().toISOString(),
    };
  });
}
