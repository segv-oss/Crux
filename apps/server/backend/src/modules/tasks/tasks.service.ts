...
import { PRTaskDTO, PaginatedResult, TaskPriority } from '../../types/index.js';
  ...
  if (cursor) {
    try {
      ...
      if (direction === 'forward') {
        conditions.push(`(t.created_at, t.id) > ($${paramIdx}::timestamptz, $${paramIdx + 1}::text)`);
      } else {
        conditions.push(`(t.created_at, t.id) < ($${paramIdx}::timestamptz, $${paramIdx + 1}::text)`);
      }
...
export async function updateTask(
  ...
  return withTransaction(async (client) => {
    ...
    if (res.rowCount === 0) {
      ...
      throw new AppError({ status: 404, code: 'TASK_NOT_FOUND', message: `Task '${taskId}' not found.` });
    }
...
export async function restoreTask(
  ...
  return withTransaction(async (client) => {
    ...
    if (checkRes.rowCount === 0) {
      throw new AppError({ status: 404, code: 'TASK_NOT_FOUND', message: `Task '${taskId}' not found on this PR.` });
    }
