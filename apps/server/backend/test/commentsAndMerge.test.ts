...
describe('Comments Hierarchy & PR Merge Guards Test Suite', () => {
  it('should enforce maximum comment nesting depth of 3', () => {
    ...
    assert.equal(canAddUnderC3, false, 'Adding comment under c3 (depth 3) must be rejected with MAX_COMMENT_DEPTH_EXCEEDED');
  });
  ...
  it('should block merge on head SHA mismatch or non-approved decision', () => {
    ...
    assert.equal(isBlockedWhenPending, true, 'Rejects with MERGE_BLOCKED_BY_REVIEW when reviewDecision is pending');
