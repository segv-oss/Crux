...
import { isRedisReady, redisPubClient, redisSubClient } from '../config/redis.js';
...
export function initializeWebSocketServer(httpServer: HttpServer): Server {
  ...
  if (isRedisReady()) {
    try {
      ...
      logger.warn(
        { err },
        'Redis adapter attachment failed, falling back to in-memory socket server',
      );
    }
  } else {
  ...
  io.on('connection', (socket: Socket) => {
    ...
    socket.on(
      'pr:join',
      async (data: { prId: string; repoId: string; lastSequenceNumber?: number }) => {
        const { prId, repoId, lastSequenceNumber } = data;

        if (!prId || !repoId) {
          socket.emit('error', {
            code: 'INVALID_PARAMETERS',
            message: 'prId and repoId are required to join room',
          });
          return;
        }
