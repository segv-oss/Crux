...
export async function seedDatabase(): Promise<void> {
  ...
  try {
    ...
    // 9. Diffs
    await client.query(`
      INSERT INTO pr_diffs (id, pr_id, file_index, path, status, additions, deletions, is_binary)
      VALUES
        ('diff_342_1', 'pr_342', 0, 'src/concurrency/redlock.ts', 'modified', 180, 20, false),
        ('diff_342_2', 'pr_342', 1, 'src/queue/outboxRelay.ts', 'modified', 45, 12, false),
        ('diff_342_3', 'pr_342', 2, 'db/migrations/0018_distributed_locks.sql', 'added', 23, 0, false)
      ON CONFLICT (id) DO NOTHING;
    `);
    ...
    await client.query(`
      ...
      VALUES
        ('rev_342_1', 'pr_342', 'usr_marcus', 'comment', 'Left notes on clock drift calculation buffer — looks good overall.', false, 1)
