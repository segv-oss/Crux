...
export function verifyHmacSignature(
  ...
  const primaryHmac = crypto
    .createHmac('sha256', config.PRIMARY_WEBHOOK_SECRET)
    .update(rawBody)
    .digest('hex');
  ...
  if (config.FALLBACK_WEBHOOK_SECRET) {
    const fallbackHmac = crypto
      .createHmac('sha256', config.FALLBACK_WEBHOOK_SECRET)
      .update(rawBody)
      .digest('hex');
...
export async function processWebhookIngress(
  ...
  try {
    ...
    logger.warn(
      { dbErr, provider, deliveryId },
      'Database write failed during webhook ingress, buffering to Redis Stream',
    );
