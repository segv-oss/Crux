...
export async function getFileDiffAST(
  repoId: string,
  prId: string,
  fileIndex: number,
): Promise<FileDiffAST> {
  ...
  if (prRes.rowCount === 0) {
    throw new AppError({
      status: 404,
      code: 'PR_NOT_FOUND',
      message: `Pull Request '${prId}' not found in repo '${repoId}'.`,
    });
  }
          ...
          ? [
              {
                ...
                oldLineNumber: 2,
                content: `- // Deleted file ${file.path}`,
              },
            ]
          : file.status === 'added'
            ? [
                {
                  ...
                  newLineNumber: 2,
                  content: `+ // Added file ${file.path} (${file.additions} lines)`,
                },
              ]
            : [
                {
                  type: 'deletion' as const,
                  oldLineNumber: 2,
                  content: `- // Old implementation of ${file.path}`,
                },
                {
                  type: 'addition' as const,
                  newLineNumber: 2,
                  content: `+ // Updated implementation of ${file.path}`,
                },
              ]),
      ],
    },
