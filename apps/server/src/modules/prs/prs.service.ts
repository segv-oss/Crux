...
export async function listPullRequests(
  ...
  if (cursor) {
    try {
      ...
      if (direction === 'forward') {
        conditions.push(
          `(pr.created_at, pr.id) < ($${paramIdx}::timestamptz, $${paramIdx + 1}::text)`,
        );
      } else {
        conditions.push(
          `(pr.created_at, pr.id) > ($${paramIdx}::timestamptz, $${paramIdx + 1}::text)`,
  ...
  const resultItems: PullRequestDTO[] = items.map((r) => ({
    ...
    checks: r.checks,
    version: r.version,
    sequenceNumber: Number.parseInt(r.sequence_number, 10),
...
export async function updatePullRequestMetadata(
  ...
  return withTransaction(async (client) => {
    ...
    const newDesc =
      body.description !== undefined ? body.description : checkRes.rows[0].description;
    const newTarget =
      body.targetBranch !== undefined ? body.targetBranch : checkRes.rows[0].target_branch;
