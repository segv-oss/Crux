...
import type { PRReviewDTO, ReviewAction } from '../../types/index.js';
  ...
  return withTransaction(async (client) => {
    ...
    if (revRes.rowCount === 0) {
      throw new AppError({
        ...
        message: `Review '${reviewId}' not found on this PR.`,
      });
    }
...
export async function listReviews(repoId: string, prId: string): Promise<PRReviewDTO[]> {
  ...
  const revRes = await pool.query(
    `SELECT id, pr_id, reviewer_id, action, comment, is_dismissed, version, created_at, updated_at
     FROM pr_reviews WHERE pr_id = $1 ORDER BY updated_at DESC`,
    [prId],
  );
