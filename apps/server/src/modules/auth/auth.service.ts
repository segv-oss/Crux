...
import { type JwtPayload, signToken } from '../../middleware/auth.js';
...
export async function exchangeGitHubCode(code: string) {
  ...
  if (config.GITHUB_CLIENT_ID && config.GITHUB_CLIENT_SECRET) {
    ...
    if (config.NODE_ENV === 'production') {
      throw new AppError({
        ...
        message:
          'Production OAuth credentials missing. GITHUB_CLIENT_ID and GITHUB_CLIENT_SECRET are required.',
      });
    }
    ...
    githubUser = {
      id: Number.parseInt(mockHash, 16) % 1000000,
  ...
  if (orgs.length === 0) {
    const defaultOrgRes = await pool.query(
      `SELECT id, name, slug FROM organizations WHERE id = 'org_crux'`,
    );
  ...
  try {
    ...
    pipeline.set(
      ...
      JSON.stringify({ userId: dbUser.id }),
      'EX',
      ttlSeconds,
    );
...
export async function getDevToken(targetUserId = 'usr_sarah') {
  let user: { id: string; email: string; name: string; avatar_url: string } = {
    id: targetUserId,
    email:
      targetUserId === 'usr_sarah'
        ? 'sarah.chen@crux.dev'
        : targetUserId === 'usr_alex'
          ? 'alex.rivera@crux.dev'
          : `${targetUserId}@crux.dev`,
    name:
      targetUserId === 'usr_sarah'
        ? 'Sarah Chen'
        : targetUserId === 'usr_alex'
          ? 'Alex Rivera'
          : 'Dana K',
    avatar_url: `https://avatars.githubusercontent.com/u/${targetUserId === 'usr_sarah' ? '102030' : '102031'}`,
  };
  ...
  try {
    ...
    // Database might be booting or unseeded; use fallback user shape
  }
  ...
  let orgs: any[] = [
    {
      ...
      name: 'crux-oss',
      slug: 'crux-oss',
      role: targetUserId === 'usr_sarah' ? 'admin' : 'member',
    },
  ];
  try {
    const orgsRes = await pool.query(
  ...
  const accessToken = signToken(jwtPayload, String(config.JWT_EXPIRES_IN));
  const refreshToken = `rf_dev_${crypto.randomUUID()}`;
