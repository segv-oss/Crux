      ...
      try {
        for (const [id, fields] of entries) {
          ...
          try {
            ...
            logger.warn(
              { dbErr, deliveryId },
              'Failed to drain buffered webhook to database, leaving in stream to retry',
            );
          }
          ...
          if (commitSuccess) {
            try {
              ...
              logger.error(
                { redisDelErr, id, deliveryId },
                'Failed to delete acknowledged entry from Redis stream',
              );
            }
          }
        }
...
export function startWebhookDrainer(intervalMs = 5000): () => void {
