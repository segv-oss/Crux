...
describe('Idempotency & Worker Fencing Test Suite', () => {
  ...
  it('should verify idempotency validation rules: user match, endpoint match, and hash match', () => {
    ...
    const isReplayValid = (userId: string, endpoint: string, body: any) => {
      if (storedRecord.user_id && userId !== storedRecord.user_id)
        return { valid: false, code: 'IDEMPOTENCY_USER_MISMATCH' };
      if (storedRecord.endpoint !== endpoint)
        return { valid: false, code: 'IDEMPOTENCY_ENDPOINT_MISMATCH' };
      if (storedRecord.request_hash !== computeRequestHash(body))
        return { valid: false, code: 'IDEMPOTENCY_PAYLOAD_MISMATCH' };
      return {
        valid: true,
        status: storedRecord.response_status,
        body: storedRecord.response_body,
      };
    };

    const validAttempt = isReplayValid('user_alice', '/api/v1/repos/repo_1/prs', {
      title: 'Add feature',
    });
    assert.equal(validAttempt.valid, true);

    // 2. Different user -> 409 IDEMPOTENCY_USER_MISMATCH
    const userMismatch = isReplayValid('user_bob', '/api/v1/repos/repo_1/prs', {
      title: 'Add feature',
    });
    ...
    const routeMismatch = isReplayValid('user_alice', '/api/v1/repos/repo_1/prs/pr_1/comments', {
      title: 'Add feature',
    });
    ...
    const payloadMismatch = isReplayValid('user_alice', '/api/v1/repos/repo_1/prs', {
