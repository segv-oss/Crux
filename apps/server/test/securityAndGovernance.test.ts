...
describe('Multi-Tenant Authorization & RBAC Test Suite', () => {
  it('should block user from accessing repository if not a member of the owning organization', () => {
    ...
    assert.equal(
      checkAccess('user_alice', 'repo_acme_core'),
      false,
      'Alice cannot access Acme repository (BOLA blocked)',
    );
    assert.equal(
      checkAccess('user_alice', 'repo_crux_backend'),
      true,
      'Alice can access Crux repository',
    );
    assert.equal(
      checkAccess('user_bob', 'repo_crux_backend'),
      false,
      'Bob cannot access Crux repository',
    );
  ...
  it('should block cross-tenant repo listing when querying unauthorized orgId', () => {
    ...
    assert.equal(
      canListOrgRepos('user_1', 'org_gamma'),
      false,
      'Should reject access to unauthorized org repository list',
    );
  });
  ...
  it('should require admin or owner role to connect new repositories to an organization', () => {
    ...
    assert.equal(
      canConnectRepo('user_member'),
      false,
      'Standard member cannot connect repositories',
    );
